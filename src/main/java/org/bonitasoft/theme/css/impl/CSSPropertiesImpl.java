...
public class CSSPropertiesImpl implements CSSProperties {
	...
	public void load(InputStream input) throws IOException {
		...
		while (cssContent.trim().length() != 0) {
			...
			CSSRule rule = new CSSRuleImpl(ruleName,comment) ;
	...
	@Override
	public void put(String rule, String key, String value) {
		CSSRule r = getRuleByName(rules,rule);
		if (r == null) {
	 ...
	 * 
	 * @param filePath path of CSS file
	...
	private List<String> toRuleNameList(List<CSSRule> ruleList) {
	...
	private CSSRule getRuleByName(List<CSSRule> ruleList,String rule) {
		Iterator<CSSRule> it = ruleList.iterator() ;
		while (it.hasNext()) {
	 ...
	 * 
	 * @param content the changed CSS content which has to write into CSS file
	 * @param filePath path of CSS file
	 * @throws CSSFileNotFoundException
	 */
    ...
    @Override	
	public String getComment (String rule){
	   CSSRule r = getRuleByName(rules,rule);
	   if ( r != null) {
	        return r.getComment();
	   }
